...
# Check events
kubectl get events -n dclaw-crisis --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-crisis deployment/dclaw-crisis-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-crisis-db-credentials -n dclaw-crisis
   ```
