...
### How do I update DClaw Crisis?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp crisis --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Crisis without Kubernetes?

Yes, for local development:

```bash
cd dclaw-crisis/frontend && npm run dev
cd dclaw-crisis/backend && uvicorn dclaw_crisis.main:app --reload
```
